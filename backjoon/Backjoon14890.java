...
//                            if( j == 0 ) System.out.println("true" + j );
                            continue;
                        }else{
//                            if( j == 0 ) System.out.println("false" + j );
                            check = false;
                            break;
                        }
                    }
...
//            if ( check ) System.out.println( i );
    ...
    public static boolean leftCheck( int currentX, int currentY ){
        for( int j=currentX-1; j>=currentX-slopeLength; j-- ){
            if( !movable( j, currentY ) ) {
                return false;
            }else if( Math.abs( map[currentY][j] - map[currentY][currentX] ) != 1 ){
                return false;
    ...
    public static void checkTotalRow(){
        for( int j=0; j<mapSize; j++ ){
            boolean check = true;
...
//                    System.out.println("@@");
                    if( map[currentY][j] - map[downY][j] > 0 ){
                        if( downCheck( j, currentY ) ){
//                            if( j == 0 ) System.out.println("true" + j );
                            continue;
                        }else{
//                            if( j == 0 ) System.out.println("false" + j );
                    ...
                    int upY = currentY-1;
                    int downY = currentY+1;
                    if( map[currentY][j] - map[upY][j] > 0 ){
                        if( upCheck( j, currentY ) ){
                            continue;
                        }else{
                            check = false;
                            break;
...
//            if ( check ) System.out.println( i );
    ...
    public static boolean upCheck( int currentX, int currentY ){
        for( int i=currentY-1; i>=currentY-slopeLength; i-- ){
            if( !movable( currentX, i ) ) {
                return false;
            }else if( Math.abs( map[i][currentX] - map[currentY][currentX] ) != 1 ){
                return false;
            }else if( visited[i][currentX] ){
                return false;
            }
            // 차이가 1이 아니면 false
            // 주어진 영역을 넘으면 false
        ...
        for( int i=currentY-1; i>=currentY-slopeLength; i-- ){
            visited[i][currentX] = true;
        }
        return true;
    }

    public static boolean downCheck( int currentX, int currentY ){
        for( int i=currentY+1; i<=currentY+slopeLength; i++ ){
            if( !movable( currentX, i ) ) {
                return false;
            }else if( Math.abs( map[i][currentX] - map[currentY][currentX] ) != 1 ){
